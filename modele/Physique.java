...
public class Physique {
	...
	private static int colision(Entite e, Niveau n) {
		...
		// ce vecteur servira a effectuer ds operation sur le mouvemant de e
		// sans influer sur ce dernier
		Point mouve = new Point(e.getStrat().deplacement.x,
				e.getStrat().deplacement.y);
		int temp = 0;
		// si e est en l'aire on augmente sa vitesse afin de simuler un saut
		// "realiste"
		if (e.getStrat().enVols)
			mouve.x = mouve.x * 2;

		// afin de ne pas tester la colision de e avec toute les entités du
		...
		// listeEntite servira a stocker les entités avec lesquelles il faudra
		...
		while (possibleX || possibleY) {
			// si il n'y a aucun deplacement en x inutils de fair les calcules
			// pour x
			if (mouve.x != 0)
				if (mouve.x > 0) {
					// calcule pour le cas ou le deplacment en x est positifs
					// donc vers la droite
					...
					if ((e.getPosX() + e.getWidth() + 1) / 25 < n.entite.length) {
						boolean b = true;
						i = 0;
						// test sur toute la taille de e, voirs si il peut
						...
						while (i < e.getHeight() && b) {
							...
							// les case du niveau ayant une taille de 25 il
						...
						if (b) {
							// si le deplacement est autoriser on actualise la
							// position de e
					...
					// calcule pour le cas ou le deplacment en x est negatifs
					// donc vers la gauche
			...
			// si il n'y a aucun deplacement en y inutils de fair les calcules
			// pour y
			if (mouve.y != 0)
				if (mouve.y > 0)
					// calcule pour le cas ou le deplacment en y est positfs
					// donc vers le bas
					// voirs explication pour droite
				...
				// calcule pour le cas ou le deplacment en y est negatifs donc
				// vers le haut
				// voirs explication pour droite
				if ((e.getPosY() - 1) / 25 > -1) {
					boolean b = true;
			...
			// flag collision
			// apres avoirs fais un deplacement de e on test si il n'est pas en
