...
public class EntiteAnime extends Entite {
	...
	public EntiteAnime(int posX, int posY, Niveau niveau, boolean solid,
			Strategie strat, Sprite sprite, int taille, int score,
	...
	public void ajouterSprite() {
		for (int i = 0; i < spritetab.length; i++) {
			spritetab[i] = new Sprite(
					this.getNiveau().stock.getSprite(i + x, y));
		}
	}
		...
		if (this.spritetab.length > 2) {
			if (this.getEtat() == 0) {
				...
				if (this.getDernierEtat() == 2 || this.getDernierEtat() == 4) {
					g.drawImage(spritetab[0].getBufferedImage(),
			...
			if (this.getEtat() == 5) {
				this.timerJoueur = 0;
				if (this.getDernierEtat() == 2 || this.getDernierEtat() == 4) {
					g.drawImage(spritetab[3].getBufferedImage(),
							((getPosX() + deltaX) * screenWidth) / 600
									+ (getWidth() * screenWidth) / 600 + 1,
							((getPosY() + deltaY) * screenHeight) / 600,
							-(getWidth() * screenWidth) / 600 + 1,
			...
			if (this.getEtat() == 1 && this.timerJoueur >= 6
					&& this.timerJoueur < 12) {
				this.setDernierEtat(1);
				g.drawImage(spritetab[1].getBufferedImage(),
						((getPosX() + deltaX) * screenWidth) / 600,
						((getPosY() + deltaY) * screenHeight) / 600,
						(getWidth() * screenWidth) / 600 + 1,
						(getHeight() * screenHeight) / 600 + 1, null);
			}
			if (this.getEtat() == 1 && this.timerJoueur >= 12
					&& this.timerJoueur < 18) {
				this.setDernierEtat(1);
				g.drawImage(spritetab[0].getBufferedImage(),
						((getPosX() + deltaX) * screenWidth) / 600,
			...
			if (this.getEtat() == 1 && this.timerJoueur >= 18) {
				this.setDernierEtat(1);
				g.drawImage(spritetab[2].getBufferedImage(),
						((getPosX() + deltaX) * screenWidth) / 600,
						((getPosY() + deltaY) * screenHeight) / 600,
						(getWidth() * screenWidth) / 600 + 1,
						(getHeight() * screenHeight) / 600 + 1, null);
			...
			if (this.getEtat() == 2 && this.timerJoueur >= 12
					&& this.timerJoueur < 18) {
				this.setDernierEtat(2);
				g.drawImage(spritetab[0].getBufferedImage(),
						((getPosX() + deltaX) * screenWidth) / 600
			...
			if ((this.getEtat() == 6 || this.getDernierEtat() == 7 || this
					.getDernierEtat() == 8) && this.timerTireur < 15) {
				...
				} else if (this.getDernierEtat() == 2
						|| this.getDernierEtat() == 8) {
					g.drawImage(spritetab[1].getBufferedImage(),
							((getPosX() + deltaX) * screenWidth) / 600,
							((getPosY() + deltaY) * screenHeight) / 600,
							(getWidth() * screenWidth) / 600 + 1,
	...
	public void drawDebug(Graphics g, int deltaX, int deltaY, int screenWidth,
		...
		if (this.spritetab.length > 2) {
			if (this.getEtat() == 0) {
				this.timerJoueur = 0;
				hitBox[0].drawDebug(g, deltaX, deltaY, screenWidth,
						screenHeight);
			}
			if (this.getEtat() == 5) {
				this.timerJoueur = 0;
				hitBox[3].drawDebug(g, deltaX, deltaY, screenWidth,
						screenHeight);
			}
			...
			if (this.getEtat() == 1 && this.timerJoueur >= 6
					&& this.timerJoueur < 12) {
				this.setDernierEtat(1);
				hitBox[1].drawDebug(g, deltaX, deltaY, screenWidth,
						screenHeight);
			}
			if (this.getEtat() == 1 && this.timerJoueur >= 12
					&& this.timerJoueur < 18) {
			...
			if (this.getEtat() == 1 && this.timerJoueur >= 18) {
				this.setDernierEtat(1);
				hitBox[2].drawDebug(g, deltaX, deltaY, screenWidth,
						screenHeight);
			}
			if (this.getEtat() == 2 && this.timerJoueur < 6) {
				this.setDernierEtat(2);
