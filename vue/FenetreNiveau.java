...
public class FenetreNiveau extends JPanel {
	...
	public FenetreNiveau(Niveau niveau){
		...
		this.getActionMap().put("released D",new ActionDeplacementReleasedD((StrategieJoueur)niveau.joueur.getStrat())  );

		this.getInputMap(WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke("pressed Q"), "pressed Q");
		this.getActionMap().put("pressed Q",new ActionDeplacementPressedQ((StrategieJoueur)niveau.joueur.getStrat())  );

		this.getInputMap(WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke("released Q"), "released Q");
		this.getActionMap().put("released Q",new ActionDeplacementReleasedQ((StrategieJoueur)niveau.joueur.getStrat())  );

		this.getInputMap(WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke("pressed Z"), "pressed Z");
		this.getActionMap().put("pressed Z",new ActionDeplacementPressedZ((StrategieJoueur)niveau.joueur.getStrat())  );
	...
	public void paint(Graphics g){
		...
		for (int i =0;i < niveau.entite.length;i++){
			for(int j = 0 ; j < niveau.entite[0].length;j++){	
				...
				if(Math.sqrt(Math.pow(e.getPosX()-niveau.joueur.getPosX(),2.0) +  Math.pow(e.getPosY()- niveau.joueur.getPosY(),2.0))<=500)
					e.rendu(g,(300 -niveau.joueur.getWidth())-niveau.joueur.getPosX(),300 -niveau.joueur.getHeight()-niveau.joueur.getPosY(),getWidth(),getHeight());
			}
		}
